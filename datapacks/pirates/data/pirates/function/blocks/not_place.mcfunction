#######################
#       Pirates       #
#      by BigSTy      #
#######################

#anchor
execute if entity @s[tag=pir.anchor.bb] run function pirates:blocks/destroy/anchor
#bundle
execute if entity @s[tag=pir.bundle.bb] run function pirates:blocks/destroy/bundle
#bottle
execute if entity @s[tag=pir.bottle.bb] run function pirates:blocks/destroy/bottle
#golden_ingot
execute if entity @s[tag=pir.golden_ingot.bb] run function pirates:blocks/destroy/golden_ingot
#many_golden_ingot
execute if entity @s[tag=pir.many_golden_ingot.bb] run function pirates:blocks/destroy/many_golden_ingots
#map1
execute if entity @s[tag=pir.map1.bb] run function pirates:blocks/destroy/map1
#map2
execute if entity @s[tag=pir.map2.bb] run function pirates:blocks/destroy/map2
#barrel_with_gold
execute if entity @s[tag=pir.barrel.bb] run function pirates:blocks/destroy/items/barrel_with_gold
#brewing barrel
execute if entity @s[tag=pir.brewing_barrel.bb] run function pirates:blocks/destroy/items/brewing_barrel
#cannon
execute if entity @s[tag=pir.cannon.bb] run function pirates:blocks/destroy/items/cannon
#cage
execute if entity @s[tag=pir.cage.bb] run function pirates:blocks/destroy/cage
#coconut
execute if entity @s[tag=pir.coconout.bb] run function pirates:blocks/destroy/coconut
#coconut
execute if entity @s[tag=pir.coconout.bb] run function pirates:blocks/destroy/coconut
#cannonball
execute if entity @s[tag=pir.cannonball.bb] run function pirates:blocks/destroy/cannonball
#gun_frame
execute if entity @s[tag=pir.gun_frame.bb] run function pirates:blocks/destroy/items/gun_frame
#candle
execute if entity @s[tag=pir.candle.bb] run function pirates:blocks/destroy/candle
#broken_lantern
execute if entity @s[tag=pir.broken_lantern.bb] run function pirates:blocks/destroy/broken_lantern
#recipe_machine
execute if entity @s[tag=pir.recipe_machine.bb] run function pirates:blocks/destroy/items/recipe_machine
#ship_in_a_bottle
execute if entity @s[tag=pir.ship_in_a_bottle.bb] run function pirates:blocks/destroy/items/ship_in_a_bottle
#golden_scull
execute if entity @s[tag=pir.golden_scull.bb] run function pirates:blocks/destroy/items/golden_scull
#golden_scull
execute if entity @s[tag=pir.shell.bb] run function pirates:blocks/destroy/shell
#golden_scull
execute if entity @s[tag=pir.cheesecake.bb] run function pirates:blocks/destroy/not_place/cheesecake